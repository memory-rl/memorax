...
class DDPPO:
    ...
    def _deterministic_action(
        self, key: Key, state: DDPPOState
    ) -> tuple[Key, DDPPOState, Array, Array, None]:
    ...
    def _stochastic_action(
        self, key: Key, state: DDPPOState
    ) -> tuple[Key, DDPPOState, Array, Array, Array]:
    ...
    def _update_actor(
        self, key, state: DDPPOState, initial_actor_carry, transitions, advantages
    ):
        ...
        def actor_loss_fn(params):
            ...
            return actor_loss - self.cfg.ent_coef * entropy, (entropy, approx_kl, clipfrac)
    ...
    def _update_critic(
        self, key, state: DDPPOState, initial_critic_carry, transitions, returns
    ):
        key, memory_key, dropout_key = jax.random.split(key, 3)
    ...
    def _update_minibatch(self, carry, minibatch: tuple):
        key, state = carry
        actor_carry, critic_carry, transitions, advantages, returns = minibatch
        key, state, critic_loss = self._update_critic(
            key, state, critic_carry, transitions, returns
        )
        key, state, actor_loss, aux = self._update_actor(
            key, state, actor_carry, transitions, advantages
        )
    ...
    def _update_epoch(self, carry: tuple):
        ...
        metrics = jax.tree.map(
            lambda x: jax.lax.pmean(x.mean(), axis_name="devices"),
            (actor_loss, critic_loss, entropy, approx_kl, clipfrac),
        )
    ...
    def _update_step(self, carry: tuple, _):
        ...
        if self.cfg.normalize_advantage:
            ...
            var = jax.lax.pmean((advantages**2).mean(), axis_name="devices") - mean**2
    ...
    def replicate(self, key, state):
        ...
        return keys, jax.device_put_replicated(state, jax.local_devices())

...
class SingleAgentBoxObsWrapper:
    """Converts token-based CoGames observations to spatial grid format.

    Same conversion as BoxObsWrapper but for PufferLibWrapper where agents
    are flattened into the environment dimension (single-agent training like IMPALA).
    """

    def __init__(self, env, grid_h: int = 11, grid_w: int = 11, num_layers: int = 24):
        self._env = env
        self._puffer_env = env._env  # Raw PufferLib vectorized env
        self._grid_h = grid_h
        self._grid_w = grid_w
        self._num_layers = num_layers
        self._grid_shape = (grid_h, grid_w, num_layers)
        ...
        self._token_obs_size = math.prod(env.obs_shape)
        self.num_envs = env.num_envs
        self.num_actions = env.num_actions
        self.obs_shape = self._grid_shape
    ...
    def reset(self, key: Key, params=None) -> Tuple[Array, PufferLibEnvState]:
        ...
        def _reset(key):
            obs, _ = self._puffer_env.reset()
            return jnp.array(obs, dtype=jnp.int32)

        tokens = jax.pure_callback(
            _reset,
            jax.ShapeDtypeStruct(self._env.obs_shape, jnp.int32),
            key,
            vmap_method="broadcast_all",
        )
        obs = self._tokens_to_grid(tokens)
    ...
    def step(
        ...
        def _step(action):
            ...
            obs, rewards, dones, truncs, infos = self._puffer_env.step(action)
            return (
                jnp.array(obs, dtype=jnp.int32),
                jnp.array(rewards, dtype=jnp.float32),

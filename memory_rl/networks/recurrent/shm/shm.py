...
class SHMCell(RNNCellBase):
  ...
  def __call__(self, carry: Array, inputs: Array) -> tuple[Array, Array]:
    ...
    if self.sample_theta and self.has_rng("cell"):
      rng = self.make_rng('cell')  # split per step when used with RNN(split_rngs=...)
      batch_shape = inputs.shape[:-1]
      idx = random.randint(rng, batch_shape, 0, self.num_thetas, dtype=jnp.int32)
      # Gather θ_t with broadcasting over batch dims: shape (*, H)
      theta_t = jnp.take(theta_table, idx, axis=0)
    else:

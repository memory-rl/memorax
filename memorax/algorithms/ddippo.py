...
This version is for multi-agent environments (JaxMarl, MettaGrid, etc.) that use
the PettingZoo-style API with env.agents, env.num_agents, and env.action_spaces.
For single-agent environments, use DDPPO instead.
...
class DDIPPOConfig:
    """Configuration for DD-IPPO algorithm.
...
@struct.dataclass(frozen=True)
class DDIPPO:
    """Decentralized Distributed Independent PPO for multi-agent environments.

    DD-IPPO trains a single model with gradients averaged across all devices,
    ...
    Assumes jittable multi-agent environments (like JaxMarl, MettaGrid, etc.)
    that use the PettingZoo-style API.

    Attributes:
        cfg: Algorithm configuration.
        env: Jittable multi-agent environment.
    ...
    def _deterministic_action(
        self, key: Key, state: DDIPPOState
    ) -> tuple[Key, DDIPPOState, Array, Array, None]:
    ...
    def _stochastic_action(
        self, key: Key, state: DDIPPOState
    ) -> tuple[Key, DDIPPOState, Array, Array, Array]:
    ...
    def _update_actor(
        self, key, state: DDIPPOState, initial_actor_carry, transitions, advantages
    ):
    ...
    def _update_critic(
        self, key, state: DDIPPOState, initial_critic_carry, transitions, returns
    ):
        key, memory_key, dropout_key = jax.random.split(key, 3)
    ...
    def replicate(self, key, state):
        ...
        return keys, replicated_state

    @partial(jax.jit, static_argnames=["self", "num_steps"])
    def warmup(self, key, state, num_steps):
        """No warmup needed for DD-IPPO."""
        return key, state
    ...
    def train(self, key, state, num_steps):
        ...
        Args:
            ...
            state: DDIPPOState (replicated across devices).

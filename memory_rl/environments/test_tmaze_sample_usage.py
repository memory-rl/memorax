...
def run_comprehensive_tests():
    ...
    actions_to_goal_right = [1]*5 + [3] + [3]  # Right to junction, Down to (L,1), Down again to stay at goal
    print_trajectory(env_passive, key_passive, actions_to_goal_right, "Passive (goal_side=1, reach goal)")

    # Passive TMaze: deterministic policy to goal (goal_side=-1)
    env_passive_left, key_passive_left = make_tmaze_env(L=5, active=False, seed=123)
    actions_to_goal_left = [1]*5 + [2] + [2]  # Right to junction, Up to (L,-1), Up again to stay at goal
    print_trajectory(env_passive_left, key_passive_left, actions_to_goal_left, "Passive (goal_side=-1, reach goal)")

    # Active TMaze: visit oracle, then go to goal (goal_side=1)
    env_active, key_active = make_tmaze_env(L=5, active=True, seed=42)
    actions_active_right = [0] + [1]*6 + [3] + [3]  # Left to oracle, right to junction (6 rights to reach x=5), down to (L,1), down again
    print_trajectory(env_active, key_active, actions_active_right, "Active (goal_side=1, reach goal)")

    # Active TMaze: visit oracle, then go to goal (goal_side=-1)
    env_active_left, key_active_left = make_tmaze_env(L=5, active=True, seed=43)
    actions_active_left = [0] + [1]*6 + [2] + [2]  # Left to oracle, right to junction (6 rights to reach x=5), up to (L,-1), up again
    print_trajectory(env_active_left, key_active_left, actions_active_left, "Active (goal_side=-1, reach goal)")

    # Random walk for both variants
    import numpy as np
    for label, (env, key) in [("Passive", (env_passive, key_passive)), ("Active", (env_active, key_active))]:
        np.random.seed(0)
...
def test_jit_compilation():
    ...
    print(f"  Reset successful: obs={timestep.observation}, pos=({int(timestep.state.x)}, {int(timestep.state.y)})")
    
    print("Testing JIT-compiled step function...")
    action = 1  # Right
    new_timestep = jit_step(timestep, action)
    print(f"  Step successful: obs={new_timestep.observation}, pos=({int(new_timestep.state.x)}, {int(new_timestep.state.y)}), reward={new_timestep.reward}")
    ...
    print(f"  Active TMaze JIT successful: obs={new_timestep_active.observation}, pos=({int(new_timestep_active.state.x)}, {int(new_timestep_active.state.y)})")

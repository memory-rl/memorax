...
class PQN:
    ...
    def train(
        ...
        def update_epoch(carry, _):
            ...
            minibatches = jax.tree_util.tree_map(
                lambda x: preprocess_transition(split_key, x), transitions
            )
            targets = preprocess_transition(split_key, lambda_targets)

            (key, state), (loss, q_value) = jax.lax.scan(
                update_minibatch, (key, state), xs=(minibatches, targets)
            )
            return (key, state, transitions, lambda_targets), (loss, q_value)

        def update_minibatch(carry, xs) -> tuple[PQNState, chex.Array, chex.Array]:
            key, state = carry
            minibatch, target = xs
        ...
        def learn(
            carry: tuple[chex.PRNGKey, PQNState], _
    ...
    def evaluate(
        ...
        def step(carry: tuple[chex.PRNGKey, PQNState], _):
            key, state = carry
...
def make_pqn(cfg, env, env_params, logger) -> PQN:
    """

"""PPO with GPT-2 architecture on POPGym.

This example demonstrates using the GPT-2 architecture as the sequence model
for a memory-requiring task. GPT-2 uses learned positional embeddings and
causal self-attention with KV caching for efficient inference.
"""
...
from memorax.networks.architectures import GPT2
...
# GPT-2 as the sequence model (torso)
torso = GPT2(
    features=64,
    num_heads=4,
    num_layers=2,
...
logger = Logger(
    [
        DashboardLogger(
            title="PPO GPT-2 RepeatPrevious",

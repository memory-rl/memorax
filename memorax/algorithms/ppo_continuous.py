...
from memorax.utils import Transition, generalized_advantage_estimatation
...
class PPOContinuous:
    ...
    def _deterministic_action(self, key, state):
        dist = self.actor.apply(state.actor_params, state.obs)
        action = dist.mode()
    ...
    def _update_actor(self, state: PPOContinuousState, transitions, advantages):
    ...
    def _update_critic(self, state: PPOContinuousState, transitions, returns):

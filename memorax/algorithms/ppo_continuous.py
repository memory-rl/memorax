...
class PPOContinuous:
    ...
    ) -> tuple[Key, PPOContinuousState, Array, Array]:
    ...
    def _step(self, carry: tuple, _, *, policy: Callable):
        ...
        transition = Transition(
            ...
            prev_action=jnp.where(state.timestep.done[..., None], jnp.zeros(self.env.action_space(self.env_params).shape[0]), state.timestep.action),  # type: ignore
    ...
    def _update_actor(
        self, key, state: PPOContinuousState, initial_actor_carry, transitions, advantages
    ):
    ...
    def _update_critic(
        self, key, state: PPOContinuousState, initial_critic_carry, transitions, returns
    ):
    ...
    def warmup(self, key, state, num_steps):
        """No warmup needed for PPOContinuous"""
        return key, state

...
def test_gradient_flow():
    ...
    y_true = jax.random.normal(jax.random.PRNGKey(42), (batch_size, seq_length, output_size))
...
def test_memory_evolution():
    ...
    for i in range(3):
        ...
        print(f"Step {i}: timestep = {current_carry.timestep}, "
              f"memory[0,0,0] = {current_carry.memory_state[0, 0, 0]}")
    
    # Check that memory changed
    assert not jnp.allclose(carry.memory_state, current_carry.memory_state), \
        "Memory state did not change"
    
    # Check that timestep incremented
    assert current_carry.timestep == 3.0, f"Expected timestep 3.0, got {current_carry.timestep}"
...
def test_overfit_simple_sequence():
        ...
        x = jax.random.normal(jax.random.PRNGKey(1), (batch_size, seq_length, input_size))
        y_true = x + 0.1 * jax.random.normal(jax.random.PRNGKey(2), x.shape)  # Simple transformation
        ...
        assert final_loss < 0.1, f"Model did not overfit simple pattern: final loss {final_loss:.6f}"

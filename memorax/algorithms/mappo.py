...
class MAPPO:
    ...
    def _update_critic(
        ...
        if self.cfg.centralized_critic:
            ...
            if self.cfg.burn_in_length > 0:
                initial_critic_carry = burn_in(
                    self.critic_network, state.critic_params, initial_critic_carry,
                    Transition(
            ...
            def critic_loss_fn(params):
                ...
                critic_loss = self.critic_network.head.loss(values, aux, returns_transformed, transitions)
                if self.cfg.clip_vloss:
                    ...
                    clipped_critic_loss = self.critic_network.head.loss(clipped_value, aux, returns_transformed, transitions)
    ...
    def warmup(self, key: Key, state: MAPPOState, num_steps: int):
        return key, state
    ...
    @partial(jax.jit, static_argnames=["self", "num_steps"])
    def evaluate(self, key: Key, state: MAPPOState, num_steps: int):

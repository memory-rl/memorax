...
class FFMCarry:
    memory_state: jnp.ndarray  # Complex-valued memory state [batch_size, memory_size, context_size]
    timestep: float  # Current timestep for gamma computation
...
class FFMCell(nn.RNNCellBase):
    ...
    def initialize_carry(self, rng: jax.random.PRNGKey, input_shape: tuple[int, ...]) -> FFMCarry:
        batch_size = input_shape[0]
        memory_state = jnp.zeros((batch_size, self.memory_size, self.context_size), dtype=jnp.complex64)
        return FFMCarry(memory_state=memory_state, timestep=0.0)
    
    def setup(self):
        a_low = -math.e
        a_high = -1e-6
        a = jnp.linspace(a_low, a_high, self.memory_size)
        b = 2 * jnp.pi / jnp.linspace(self.min_period, self.max_period, self.context_size)
    ...
    def __call__(
        ...
        """
        assert inputs.ndim == 2, f"Input must be 2D (batch_size, input_size), got {inputs.ndim}D"
        assert inputs.shape[1] == self.input_size, f"Input size must be {self.input_size}, got {inputs.shape[1]}"
        ...
        z_in = jnp.concatenate([jnp.real(new_memory_state), jnp.imag(new_memory_state)], axis=-1)
    ...
    def _apply_ffa_single_step(
        ...
        a = jnp.clip(jnp.reshape(a, (1, -1, 1)), a_max=-1e-6)# [memory_size] -> [1, memory_size, 1]
        ...
        x_complex = jnp.broadcast_to(x_complex, prev_state.shape)  # [batch_size, memory_size, context_size]
...
if __name__ == "__main__":
    ...
    print(f"New timestep: {new_carry.timestep}")
